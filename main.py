...
<div class="card s2"><div class="label"><span class="posAvatar">🐧</span> 현재 포지션</div><div id="posMini" class="posMini"><span class="muted">-</span></div></div>
...
function renderPositionMini(bg){
  ...
  let summary=`<div class="muted" style="margin-top:6px;font-size:11px;line-height:1.6">승률 ${winRate!=null?winRate+'%':'-'} · PNL(통합) <span class="${combinedCls}">${Number.isFinite(combined)?(combined>=0?'+':'')+n(combined):'-'}</span><br>총 USDT ${Number.isFinite(equity)?n(equity):'-'}</div>`;
