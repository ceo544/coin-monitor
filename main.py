...
EXPORT_TOKEN_PATHS = {"/export.csv", "/export-signals.csv"}
...
def _require_login() -> Any:
    if request.path in PUBLIC_PATHS:
        return None
    if request.path in EXPORT_TOKEN_PATHS:
        token = request.args.get("token", "")
        expected = get_setting("EXPORT_API_TOKEN")
        if token and expected and hmac.compare_digest(token, expected):
            return None  # valid automation token - e.g. the daily GitHub Actions export job
    if session.get("user_id"):
        return None
    if request.path.startswith("/api/") or request.path in EXPORT_TOKEN_PATHS:
...
<div class="card" style="margin-top:16px"><h2>📦 자동 데이터 백업 (GitHub Actions) <span class="hint" style="margin:0">(모든 회원 공통)</span></h2>
<div class="hint" style="margin-top:-6px;margin-bottom:10px">매일 자동으로 새로 쌓인 데이터만 GitHub 저장소에 커밋해서 누적시키고 싶다면, 아래 토큰을 GitHub 저장소의 Secrets에 등록하세요 (설정 방법은 README 참고). 로그인 비밀번호가 아니라 이 용도로만 쓰는 별도 토큰이라 안전합니다.</div>
<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
<input type="text" id="exportTokenBox" readonly value="{EXPORT_TOKEN}" style="flex:1;min-width:260px;font-family:monospace;background:#0a172b;border:1px solid var(--line);border-radius:8px;padding:9px 11px;color:var(--text)">
<button type="button" id="copyTokenBtn" class="btn" style="padding:9px 14px">복사</button>
<button type="button" id="regenTokenBtn" class="btn" style="padding:9px 14px;background:#3a2030">재발급</button>
</div>
<div class="hint" style="margin-top:8px">CSV 다운로드 주소: <code>https://당신의도메인/export.csv?token=발급받은토큰&since_id=마지막ID</code></div>
</div>
...
document.getElementById('copyTokenBtn').onclick = () => {
  document.getElementById('exportTokenBox').select();
  navigator.clipboard.writeText(document.getElementById('exportTokenBox').value).then(()=>alert('복사했습니다.'));
};
document.getElementById('regenTokenBtn').onclick = async () => {
  if (!confirm('토큰을 재발급하면 예전 토큰은 더 이상 안 됩니다 (GitHub Secrets도 같이 업데이트해야 함). 계속할까요?')) return;
  const r = await fetch('/api/export-token/regenerate', {method: 'POST'});
  const j = await r.json();
  if (j.ok) { document.getElementById('exportTokenBox').value = j.token; alert('재발급 완료. GitHub Secrets도 새 값으로 업데이트해주세요.'); }
...
def settings_page() -> str:
    ...
    out = out.replace("{EXPORT_TOKEN}", html.escape(get_or_create_export_token(), quote=True))
    for group, content in rendered.items():
        out = out.replace("{" + group + "}", content)
    return out


@app.post("/api/export-token/regenerate")
def api_regenerate_export_token() -> Response:
    new_token = secrets.token_hex(24)
    save_setting("EXPORT_API_TOKEN", new_token)
    log("SETTINGS", f"export API token regenerated by user {session.get('username')}")
...
def get_or_create_export_token() -> str:
    """Read-only automation token (e.g. for a daily GitHub Actions job) that
    grants access to /export.csv and /export-signals.csv without needing a
    real login session - separate from any user's actual password, so it's
    safe to hand to CI. Global (not per-user) since the observations data
    itself is shared/global, not user-specific."""
    token = get_setting("EXPORT_API_TOKEN")
    if not token:
        token = secrets.token_hex(24)
        save_setting("EXPORT_API_TOKEN", token)
    return token
...
def _build_signal_message(
    ...
    # Same entry-quality grade shown on the dashboard (spec: real 09/15
    # incident where a 210-minute-old signal + higher-timeframe conflict
    # got mistaken for a fresh entry) - included here too so it's visible
    # right in the alert, not only if the person happens to check the
    # dashboard.
    indicators = (binance_snapshot or {}).get("indicators") or {}
    risk = assess_entry_risk(side, minutes_since_start, indicators)
    if risk.get("grade") or risk.get("minutes_since_start") is not None or risk["flags"]:
        ...
        for f in risk["flags"]:
            lines.append(flag_labels.get(f, f))
...
def _maybe_notify_telegram(
    ...
    binance_snapshot: Optional[Dict[str, Any]] = None,
    dashboard_url: str = "",
    minutes_since_long: Optional[float] = None,
    minutes_since_short: Optional[float] = None,
) -> None:
    if not bot_token or not chat_id:
        return
        ...
        if long_active or notify_off:
            try:
                ...
                send_telegram_message(_build_signal_message(
                    ...
                    minutes_since_start=minutes_since_long,
        ...
        if short_active or notify_off:
            try:
                ...
                send_telegram_message(_build_signal_message(
                    ...
                    minutes_since_start=minutes_since_short,
                ), bot_token, chat_id, user_id)
            except Exception as exc:
        ...
        for uid in user_ids:
            ...
            try:
                ...
                _maybe_notify_telegram(
                    ...
                    minutes_since_long=minutes_since_long,
                    minutes_since_short=minutes_since_short,
                )
            except Exception as exc:
...
def api_telegram_test() -> Response:
    ...
    if side and parsed.get("signals"):
        ...
        minutes_since_start = _minutes_since(
            latest.get("long_start") if side == "long" else latest.get("short_start"),
            latest.get("observed_at"),
        )
        preview = _build_signal_message(
            side, True, parsed, latest.get("current_price_raw"), color, binance_snapshot,
            dashboard_url=settings["DASHBOARD_URL"], minutes_since_start=minutes_since_start,
        )
...
<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px">
  <button type="button" class="btn ivBtn" data-iv="1m" style="padding:5px 10px;font-size:11px">1분</button>
  <button type="button" class="btn ivBtn" data-iv="5m" style="padding:5px 10px;font-size:11px">5분</button>
  <button type="button" class="btn ivBtn active" data-iv="15m" style="padding:5px 10px;font-size:11px">15분</button>
  <button type="button" class="btn ivBtn" data-iv="1h" style="padding:5px 10px;font-size:11px">1시간</button>
...
document.querySelectorAll('.ivBtn').forEach(btn=>{
  btn.onclick=()=>{
...
function setTickerSpeed(track,pxPerSec){
  // Duration derived from actual content width so the scroll always feels
  // like the same, slow, easy-to-read pace - regardless of whether there
  // happen to be 3 headlines or 30 at any given moment. requestAnimationFrame
  // is used to wait one paint so scrollWidth reflects the just-set innerHTML.
  requestAnimationFrame(()=>{
    let full=track.scrollWidth/2; // content is duplicated for the seamless loop
    let duration=Math.max(20,full/pxPerSec);
...
async function refreshCoinTicker(){
  try{
    ...
    let html=items.map(it=>{
      ...
      return `<span class="coinItem"><b>${esc(sym)}</b> ${n(it.price)} <span class="chgPct ${up?'statusUp':'statusDown'}">${up?'▲':'▼'}${Math.abs(it.change_pct).toFixed(2)}%</span></span>`;
    }).join('');
    track.innerHTML=html+html;
    setTickerSpeed(track,45);
  }catch(e){/* ticker is non-critical; fail silently */}
